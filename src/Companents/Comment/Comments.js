...
import '../assistents/css/comments.css';
...
export default function Comments(){
  ...
  console.log(value)
    useEffect(()=>{
      axios.get(`http://127.0.0.1:8000/api/posts/comment/all/${value.id}`,
  ...
  function get_message(message){
    ...
    if(message !==''){
      ...
      axios.post(`http://127.0.0.1:8000/api/posts/comment/create/${value.id}`,send_message,config)
    .then(data=>{
      if(data.status ===201){
        const date_message= []
  ...
  <>
      ...
      <section className="chat-area">
        ...
        <><header>
          ...
          <img className='commentimage' src={`http://127.0.0.1:8000${isinfo.post_image}`} alt=""/>
        
        </header>
        <p style={{textAlign:'center'}}>{isinfo.post}</p></>
        : <h1>No found</h1>}
