...
import '../assistents/css/profileimage.css';
...
export default function ProfileImage(){
   ...
   const value = useParams();
    ...
    useEffect(()=>{
        axios.get(`http://127.0.0.1:8000/api/users/image/list/${value.id}`,{
    ...
    function dot(index){
        ...
        dates.forEach(function(date,index){
            ...
            console.log(document.getElementById("item" + index),"shexroz")
    ...
    function delete_image(){
        ...
        axios.delete(`http://127.0.0.1:8000/api/users/image/delete/${+images}`,{
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        })
        .then(data=>{
            console.log(data.data)
    ...
    return(
       ...
       <Link to={`/profile/${value.id}`}>
           <button onClick={()=>{main_page()}} style={{position:'relative',left:'10%',backgroundColor:'green',textAlign:'center'}} className='btn btn-primary'>Exit</button></Link></>}
        ...
        isallimage.map((item,index)=>(
         ...
         : `0${new Date(item.date).getMinutes()}`} created profile image</p>
            <img className={index === 0 ?'imgclass active' :'imgclass'} src={`http://127.0.0.1:8000${item.photo}`} id={`content${index+1}`} alt={`${item.id}`} /></>
    ...
    {isallimage.length ?
            isallimage.map((item,index)=>(
                <button style={{display:"none"}} onClick={()=>{dot(index+1)}}></button>
            )) : 'no'}
        </div>
        </>
